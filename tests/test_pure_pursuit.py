...
class TestPurePursuit(unittest.TestCase):
    def test_load_params(self):
        ...
        planner = PurePursuitPlanner(track=track, params=params)
        vgain = planner.params["vgain"]
        self.assertTrue(abs(vgain - params["vgain"]) < 1e-6,
        ...
        planner = PurePursuitPlanner(track=track, params=filepath)
        expect_vgain = load_params(default_params={}, new_params=filepath)["vgain"]
        self.assertTrue(abs(planner.params["vgain"] - expect_vgain) < 1e-6,
                        f"vgain from yaml params is not correct, got {planner.params['vgain']} != {expect_vgain}")

        # none params
        expect_vgain = 1.0
        planner = PurePursuitPlanner(track=track, params=None)
        self.assertTrue(abs(planner.params["vgain"] - expect_vgain) < 1e-6,
