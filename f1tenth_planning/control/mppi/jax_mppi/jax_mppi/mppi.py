...
class MPPI(MPC):
  ...
  def __init__(self, n_iterations=5, n_steps=16, n_samples=16, temperature=0.01,
    ...
    self.state_rollouts = None
    if ref_traj_fn is None:
      self.get_reference_trajectory = lambda x: jnp.zeros((self.n_steps, 1))
    else:
      self.get_reference_trajectory = ref_traj_fn
  ...
  def init_state(self, a_shape, rng):
    ...
    dim_a = jnp.prod(jnp.array(a_shape))  # np.int32
  ...
  def update(self, mpc_state, env, env_state, rng, reward_fn=None,
    ...
    dim_a = jnp.prod(jnp.array(env.a_shape))  # np.int32
    ...
    def iteration_step(input_, _):
      ...
      s, r, a = jax.vmap(self.rollout, in_axes=(0, None, None, None, None, None))(
      ...
      return (a_opt, a_cov, rng), s
    if not self.scan:
      for _ in range(self.n_iterations):
        (a_opt, a_cov, rng), rollouts = iteration_step((a_opt, a_cov, rng), None)
    else:
      (a_opt, a_cov, rng), rollouts = jax.lax.scan(
          iteration_step, (a_opt, a_cov, rng), None, length=self.n_iterations
      )
    return (a_opt, a_cov), rollouts

  def get_action(self, mpc_state, a_shape):
    a_opt, _ = mpc_state
    return jnp.reshape(a_opt[0, :], a_shape)

  def get_mppi_output(self, env, curr_state, env_state):
    a_opt, _ = env_state
    is_terminal = jnp.zeros((self.n_steps, 1))
    is_terminal = is_terminal.at[-1].set(1)
    ref_traj = self.get_reference_trajectory(curr_state)
    def rollout_step(traj_state, step_state):
      a, ref_state, is_terminal = step_state
      a = jnp.reshape(a, env.a_shape)
      traj_state = env.step(traj_state, a)
      r = env.reward(traj_state, a, ref_state, is_terminal)
      return traj_state, (traj_state, r)
    _, (s_opt, r_opt) = jax.lax.scan(rollout_step, curr_state, (a_opt, ref_traj, is_terminal))
    R_opt = self.returns(r_opt)
    return s_opt, R_opt, a_opt
  ...
  def rollout(self, actions, env, env_state, reward_fn=None,
    ...
    def rollout_step(env_state, step_state):
      a, ref_state, is_terminal = step_state
      a = jnp.reshape(a, env.a_shape)
      env_state = env.step(env_state, a)
      r = env.reward(env_state, a, ref_state, is_terminal)
      return env_state, (env_state, r)
    
    is_terminal = jnp.zeros((self.n_steps, 1))
    is_terminal = is_terminal.at[-1].set(1)
    
    ref_traj = self.get_reference_trajectory(env_state)

    if not self.scan:
      # python equivalent of lax.scan
      scan_output = []
      for t in range(self.n_steps):
        env_state, output = rollout_step(env_state, actions[t, :], ref_traj[t,:], is_terminal[t])
        scan_output.append(output)
      s, r = jax.tree_util.tree_map(lambda *x: jnp.stack(x), *scan_output)
    else:
      _, (s, r) = jax.lax.scan(rollout_step, env_state, (actions, ref_traj, is_terminal))
